...
function showHelp(sid, msgId) {
  const lines = [
    ...
    '─'.repeat(20),
    '── 会话管理 ──',
    '/list (/l,/ls,/sessions)  查看列表；/l all [+N] 全部',
    '/switch (/s,/sw) <N|ID>   切换会话',
    '/new (/nl,/create) <名称> 新建并切换',
    ...
    '/pd   PAD 模式（摘要显示，推荐）',
    '/ph   PHONE 模式（极简显示）',
    `/quota (/q)  超长策略 (当前: ${quotaModeLabel(quotaMode)})`,
    '              t截断 n通知 c续传',
    '/g (/cont)  续发模式下一条（显示进度）',
    '/x (/gc)    清除待续发内容',
    '',
    ...
    '💡 未识别的消息转发给当前 AI 会话',
    '',
    '⚠️ FULL mode context_token 约 5 次 sendmessage 上限',
    '   超限自动截断，/quota c 启用续传 + /g 逐条取出',
    '   推荐 PAD 或 Web UI (http://localhost:4096)',
  ];
  reply(sid, lines.join('\n'));
  sendResponse(msgId, { stopReason: 'end_turn' });
}
