...
超长消息自动分段发送。iLink API 对每个 `context_token` 有约 5 次 `sendmessage` 配额，超过后回复会被截断。可通过 `/quota` 命令设置超长回复策略。
...
> 
> **超长回复策略：** 可通过 `/quota` 设置。`truncate`（默认）静默截断超限部分；`notify` 截断时通知用户；`continue` 保留剩余文本，发任意消息后自动续发。
...
| 命令 | 别名 | 说明 |
|------|------|------|
| `/quota [策略]` | `/q` | 查看/设置超长回复策略 (truncate\|notify\|continue) |
| `/mute` | `/m` | 开关主动通知（默认开启） |
