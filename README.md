...
| **思考/推理** | 实时流式显示 `🤔 Thinking...` → 思考内容 → `✅ Thinking complete (Xs)` | 折叠为 `+ Thinking: X.Xs`（仅显示时长） | 完全隐藏 |
| **工具调用** | 实时显示 `🔧 tool_name [input]` → `✅ tool_name`（含输出摘要） | 折叠为 `⚙tool_name [input]`（状态变更时通知） | 完全隐藏 |
