...
- **FULL**：实时流式输出，包含推理过程、工具调用和文件差异
- **PAD**：摘要显示，仅累积文本回复，隐藏工具输出和文件细节
...
| `/f` | — | FULL 模式：显示所有信息（工具调用、文件差异等全部转发） |
| `/pd` | — | PAD 模式：摘要显示，隐藏工具输出和文件细节（**默认，推荐**） |
| `/ph` | — | PHONE 模式：极简模式，仅显示 AI 文本回复和错误 |

> ⚠️ **FULL 模式不推荐日常使用。** 该模式通过微信实时推送每个文本/工具增量，在长文本生成时微信 API 会触发限流，网络波动也可能导致部分消息丢失，最终收到的回复可能不完整。建议优先使用 PAD 模式，或通过 OpenCode Web UI (http://localhost:4096) 查看完整输出。

FULL 模式下支持实时流式输出，逐段推送 AI 文本和工具调用状态。
